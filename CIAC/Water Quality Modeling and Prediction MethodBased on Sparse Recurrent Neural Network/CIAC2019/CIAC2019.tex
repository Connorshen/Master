...
This study selected the water data of a reservoir in Zhejiang 
from May 2012 to May 2015. The data source was the actual value
...
and total nitrogen, were selected to evaluate the water quality rank. 
...
in the morning  was selected. 
Considering the possible abnormalities of the water quality 
parameters in the measurement, the data was preprocessed to remove 
some obvious abnormal data. It used the Grubbs criterion for outlier 
detection. First of all, the data is sorted in ascending order. 
The outliers were sequentially removed from both ends until the data 
set meets the requirements. In order to determine the abnormal data, 
it needed to be calculated:
...
Since the data and the detection range of the water indices were not 
uniform, the input data needed to be normalized. The value of the 
input data x was mapped between 0.2 and 0.8. The normalization formula is as follows:
\begin{equation}
y=0.6*\frac{x-min(x)}{max(x)-min(x)}+0.2  
\end{equation}

The minimum time unit of the data set was day $T$. $T$ was taken form 7 days, 14 days, 
and 30 days. They respectively generate 3 sets of training and 
testing data sets. The water quality data from 
May 1, 2012 to April 30, 2014 were selected as training data. 
The water quality data  from May 1, 
2014 to April 30, 2015 were selected as testing data.
...
Water quality data for several days were used as input to 
predict water quality afterwards parameters. Network performance 
was quantified by calculating the root mean square (RMS) error:
...
The number of recurrent layer neurons was set to 1000.
The weight intensity coefficient $g$ was set to 1.5. 
The step size of Equation 1 was set to 0.05. 
The constant $\alpha$ of the learning rate matrix $P$ was set to 1.5.
\begin{figure}[htbp]
\centering
...
Figure \ref{RMS error for SRNN} shows the learning curve of SRNN, that was the changes of 
RMS with the number of iterations. The data in the figure was the 
...
It converged after about 15 iterations, which indicates that the training 
...
indicators in Figure \ref{Prediction results for ammonia nitrogen
and dissolved oxygen using SRNN} against the test data set were 0.065 and 0.057.
